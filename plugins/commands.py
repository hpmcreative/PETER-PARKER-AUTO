...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
             InlineKeyboardButton('🚫 Dᴏɴᴛ Cʟɪᴄᴋ Hᴇʀᴇ 🚫', callback_data='close_data')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAECchllMVvLKRL91hr9gybdoMe_wdYeBAAC_gEAAiM73SaiLda6MsIEKR4E") 
    ...
    await client.send_cached_media(
        ...
        file_id=file_id,
        caption=f_caption,
        reply_markup=InlineKeyboardMarkup( [ [ InlineKeyboardButton('Cʜᴇᴄᴋ Mᴏʀᴇ', url=(MAIN_CHANNEL)) ] ] ),
        protect_content=True if pre == 'filep' else False,
        )
